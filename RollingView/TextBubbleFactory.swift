...
class ContextBubbleFactory {
	...
	func create(width: CGFloat, isRightSide: Bool, title: String, attr: VideoAttributes) -> CALayer {
		...
		top = topLabel.frame.bottom + 11

		let tags = createTags(width: innerWidth, attr: attr)
		tags.frame.left = CONTEXT_MASTER_INSETS.left
		tags.frame.top = top
		master.addSublayer(tags)
		top = tags.frame.bottom + 11

		master.frame.size.height = tags.frame.bottom + CONTEXT_MASTER_INSETS.bottom

		return master
	}


	private func createTags(width: CGFloat, attr: VideoAttributes) -> CALayer {
		let scale: CGFloat = 1
		let host = CALayer()
		host.masksToBounds = true
		// host.backgroundColor = UIColor.lightGray.cgColor
		host.frame = CGRect(x: 0, y: 0, width: width, height: 0)
		host.sublayers = attr.tags.map({ (tagId) -> CALayer in
			createTag(scale: scale, title: tagId, color: PredefinedTags.defaultColor)
		})
		host.frame.size.height = arrangeVertically(scale: scale, width: width, layers: host.sublayers!)
		return host
	}


	private func arrangeVertically(scale: CGFloat, width: CGFloat, layers: [CALayer]) -> CGFloat {
		var top: CGFloat = 0
		var left: CGFloat = 0
		for layer in layers {
	...
	private func createTag(scale: CGFloat, title: String, color: UIColor, showIcon: Bool = true) -> CALayer {
		...
		label.frame.top = (host.frame.height - label.frame.height) / 2
